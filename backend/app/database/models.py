...
class Document(Base):
    ...
    external_sync_status = Column(String(50), default='UNSYNCED', index=True)
    external_sync_ref = Column(String(100), nullable=True, index=True)
    ...
    __table_args__ = (Index('ix_documents_assigned_status', 'assigned_approver', 'status'), Index('ix_documents_division_status', 'division', 'status'), Index('ix_documents_created_status', 'created_at', 'status'))
Invoice = Document
...
class WorkflowProfile(Base):
    ...
    rule_action = Column(String(50), default='WORKFLOW_ROUTE', index=True)
...
class WorkflowStepDefinition(Base):
    ...
    approver_type = Column(String(100), default='Approval Pool')
...
class BusinessRule(Base):
    ...
    target_workflow_id = Column(String(200), nullable=False, index=True)
...
class ThirdPartyWebhookConfig(Base):
    ...
    is_active = Column(Boolean, default=True, index=True)
    events_json = Column(Text, default='["document.settled"]')
...
class IntegrationSyncLog(Base):
    ...
    sync_direction = Column(String(20), default='PUSH', index=True)
    target_system = Column(String(100), default='WEBHOOK', index=True)
...
class CallbackRule(Base):
    ...
    trigger_event = Column(String(100), default='FDO_FINAL_DECISION')
    run_when = Column(String(30), default='BOTH')
    conditions_json = Column(Text, nullable=True)
    http_method = Column(String(15), default='POST')
    url_mode = Column(String(20), default='INHERIT_BASE')
    endpoint_path = Column(String(500), nullable=True)
    custom_url = Column(String(500), nullable=True)
    body_type = Column(String(30), default='JSON')
    payload_source = Column(String(50), default='MAPPING')
    stored_procedure_name = Column(String(200), nullable=True)
    ...
    auth_override_type = Column(String(50), default='INHERIT')
