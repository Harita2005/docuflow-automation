...
def build_document_context(doc: Document, decision: str='APPROVED', event_id: str='') -> Dict[str, Any]:
    ...
    return {'primaryKey': str(primary_key), 'documentNumber': str(doc_number), 'approvalStatus': str(decision).upper(), 'documentType': str(doc_type), 'applicationCode': '', 'company': str(company), 'category': str(category), 'branch': str(branch), 'costCenter': str(cost_center), 'pay_mode': str(pay_mode), 'approvalStage': str(doc.current_stage), 'status': str(doc.status), 'fdoDecision': str(decision).upper(), 'approvedBy': str(approved_by), 'approvalDate': approval_date, 'rejectionReason': str(rejection_reason), 'eventId': str(event_id), 'amount': float(doc.amount) if doc.amount is not None else 0.0, 'party_name': doc.party_name or doc.vendor_name or '', 'party_code': doc.party_code or doc.vendor_code or ''}
...
def resolve_dynamic_variables(template_str: str, context: Dict[str, Any]) -> str:
    ...
    for key, val in context.items():
        snake_key = re.sub('(?<!^)(?=[A-Z])', '_', key).lower()
        ...
        if auth_type == 'API_KEY':
            ...
            if not access_token and client_id and client_secret and token_url:
                try:
                    ...
                    auth_data = urllib.parse.urlencode({'grant_type': 'client_credentials', 'client_id': client_id, 'client_secret': client_secret, 'scope': cfg.get('scope', '')}).encode('utf-8')
...
def execute_sp_for_callback_payload(db: Optional[Session], sp_name: str, doc_key: str, doc_context: Dict[str, Any]) -> str:
    ...
    """
    clean_sp_name = re.sub('[^a-zA-Z0-9_]', '', (sp_name or 'sp_GetApprovalCallbackPayload').strip()) or 'sp_GetApprovalCallbackPayload'
    if db:
        try:
    ...
    try:
        doc = None
        if db:
            doc = db.query(Document).filter((Document.id == str(doc_key)) | (Document.doc_key == str(doc_key)) | (Document.invoice_number == str(doc_key))).first()
        line_items = []
        if doc and doc.line_items:
            for item in doc.line_items:
                line_items.append({'itemCode': item.item_code or 'ITEM-001', 'itemDescription': item.description, 'quantity': float(item.quantity or 1.0), 'unitPrice': float(item.unit_price or 0.0), 'lineAmount': float(item.amount or 0.0)})
...
def build_callback_request(rule: CallbackRule, app: ThirdPartyApplication, doc_context: Dict[str, Any], db: Optional[Session]=None) -> Tuple[str, str, Dict[str, str], Optional[bytes]]:
    ...
    if query_params:
        ...
        final_url = urllib.parse.urlunparse((parsed_url.scheme, parsed_url.netloc, parsed_url.path, parsed_url.params, new_query_str, parsed_url.fragment))
    final_headers = {'User-Agent': 'DocuFlow-Approval-Callback-Engine/2.0', 'X-Approval-Event-ID': doc_context.get('eventId', ''), 'X-Approval-Attempt': f"{doc_context.get('attemptNumber', 1)}/{doc_context.get('maxAttempts', 3)}"}
...
def execute_callback_event(db: Session, event_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(url=final_url, data=body_bytes, headers=final_headers, method=method)
        ...
        attempt = CallbackAttempt(callback_event_id=event.id, attempt_number=attempt_num, http_method=rule.http_method or 'POST', request_url=rule.custom_url or app.base_url, request_headers_json=json.dumps(mask_sensitive_headers(build_auth_headers(app.auth_type, app.auth_config_json))), request_body=None, response_status_code=status_code, response_body=resp_body[:4000], response_time_ms=elapsed_ms, status='FAILED', error_message=last_error)
        db.add(attempt)
    except Exception as ex:
        elapsed_ms = int((time.time() - start_time) * 1000)
        last_error = str(ex)
        attempt = CallbackAttempt(callback_event_id=event.id, attempt_number=attempt_num, http_method=rule.http_method or 'POST', request_url=rule.custom_url or app.base_url, request_headers_json=json.dumps(mask_sensitive_headers(build_auth_headers(app.auth_type, app.auth_config_json))), request_body=None, response_status_code=status_code, response_body=None, response_time_ms=elapsed_ms, status='FAILED', error_message=last_error)
        db.add(attempt)
    max_attempts = event.max_attempts or 3
    if rule.retry_config_json:
        try:
...
def dispatch_approval_callback_events(db: Session, document_id: str, decision: str) -> List[Dict[str, Any]]:
    ...
    if not doc:
        return []
    rules = db.query(CallbackRule).join(ThirdPartyApplication).filter(CallbackRule.status == 'ACTIVE', ThirdPartyApplication.status == 'Active', CallbackRule.trigger_event == 'FDO_FINAL_DECISION', CallbackRule.run_when.in_([decision.upper(), 'BOTH'])).order_by(CallbackRule.priority.asc()).all()
    if not rules:
        return []
    results = []
    ...
    for rule in rules:
        ...
        event = CallbackEvent(event_id=event_id_str, document_id=doc.id, rule_id=rule.id, application_id=app.id, source_primary_key=doc_context['primaryKey'], document_number=doc_context['documentNumber'], decision=decision.upper(), status='PENDING', attempt_count=0, max_attempts=3)
        db.add(event)
        db.commit()
        db.refresh(event)
        res = execute_callback_event(db, event.id)
        results.append({'event_id': event_id_str, 'application': app.name, 'rule': rule.rule_name, 'result': res})
    return results