...
async def get_current_user(credentials: Optional[HTTPAuthorizationCredentials]=Depends(security), db: Session=Depends(get_db)) -> Optional[User]:
    if not credentials:
    ...
    if token_session_id and user.active_session_id and (user.active_session_id != token_session_id):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='SESSION_TERMINATED_BY_NEW_LOGIN', headers={'WWW-Authenticate': 'Bearer', 'X-Session-Status': 'Terminated'})
    return user