...
try:
    ...
    try:
        if db_init.query(User).count() == 0:
            ...
            core_users = [User(id=1, user_uid='U-ADMIN', employee_id='EMP001', employee_name='System Administrator', name='System Administrator', username='admin', email='harita010905@gmail.com', role='admin', password_hash=hashed_pass, is_active=True, is_deleted=False), User(id=214, user_uid='U-214', employee_id='EMP214', employee_name='VIGNESH M', name='VIGNESH', username='VIGNESH', email='vignesh.m@ramrajcotton.net', role='manager', password_hash=hashed_pass, is_active=True, is_deleted=False), User(id=222, user_uid='U-222', employee_id='EMP222', employee_name='YUVASREE', name='YUVASREE', username='YUVASREE', email='wmssupport@ramrajcotton.net', role='employee', password_hash=hashed_pass, is_active=True, is_deleted=False), User(id=1002, user_uid='U-1002', employee_id='EMP1002', employee_name='VARUNAN R', name='VARUNAN', username='VARUNAN', email='varunan.r@ramrajcotton.net', role='employee', password_hash=hashed_pass, is_active=True, is_deleted=False), User(id=1226, user_uid='U-1226', employee_id='EMP1226', employee_name='NATTUDURAI S', name='Nattudurai', username='Nattudurai', email='Nattudurai.s@ramrajcotton.net', role='manager', password_hash=hashed_pass, is_active=True, is_deleted=False)]
            for u in core_users:
                db_init.add(u)
            db_init.commit()
    ...
    if is_mssql:
        ...
        doc_cols = [('party_name', 'NVARCHAR(250) NULL'), ('party_code', 'NVARCHAR(100) NULL'), ('party_tax_id', 'NVARCHAR(50) NULL'), ('vendor_name', 'NVARCHAR(250) NULL'), ('vendor_code', 'NVARCHAR(100) NULL'), ('vendor_gstin', 'NVARCHAR(50) NULL'), ('cgst', 'FLOAT NULL'), ('sgst', 'FLOAT NULL'), ('igst', 'FLOAT NULL'), ('is_deleted', 'BIT NOT NULL DEFAULT 0'), ('deleted_at', 'DATETIME NULL'), ('pi_indicator', 'NVARCHAR(10) NULL'), ('trans_type', 'NVARCHAR(20) NULL'), ('gstin', 'NVARCHAR(15) NULL'), ('doc_status', 'INT NULL DEFAULT 0'), ('doc_due_date', 'DATE NULL'), ('contact_person', 'NVARCHAR(100) NULL'), ('pay_mode', "NVARCHAR(10) NOT NULL DEFAULT N'BANK'"), ('link_column', 'NVARCHAR(500) NULL'), ('external_sync_status', "VARCHAR(50) NULL DEFAULT 'UNSYNCED'"), ('external_sync_ref', 'VARCHAR(100) NULL'), ('external_synced_at', 'DATETIME NULL'), ('external_sync_system', 'VARCHAR(100) NULL'), ('external_sync_error', 'NVARCHAR(MAX) NULL')]
        ...
        user_cols = [('is_deleted', 'BIT NOT NULL DEFAULT 0'), ('deleted_at', 'DATETIME NULL'), ('mfa_enabled', 'BIT NOT NULL DEFAULT 0'), ('mfa_type', "VARCHAR(50) NULL DEFAULT 'EMAIL'"), ('mfa_secret', 'VARCHAR(100) NULL'), ('last_login', 'DATETIME NULL'), ('active_session_id', 'VARCHAR(100) NULL'), ('active_device_info', 'NVARCHAR(255) NULL'), ('session_created_at', 'DATETIME NULL'), ('last_activity_at', 'DATETIME NULL')]
        ...
        wf_cols = [('is_deleted', 'BIT NOT NULL DEFAULT 0'), ('deleted_at', 'DATETIME NULL'), ('rule_action', "VARCHAR(50) NOT NULL DEFAULT 'WORKFLOW_ROUTE'"), ('cancel_reason', 'NVARCHAR(MAX) NULL'), ('auto_approve_enabled', 'BIT NOT NULL DEFAULT 0'), ('auto_approve_condition', 'NVARCHAR(MAX) NULL'), ('auto_cancel_enabled', 'BIT NOT NULL DEFAULT 0'), ('auto_cancel_condition', 'NVARCHAR(MAX) NULL')]
        ...
        rule_cols = [('is_deleted', 'BIT NOT NULL DEFAULT 0'), ('deleted_at', 'DATETIME NULL'), ('rule_action', "VARCHAR(50) NOT NULL DEFAULT 'WORKFLOW_ROUTE'"), ('cancel_reason', 'NVARCHAR(MAX) NULL'), ('auto_approve_enabled', 'BIT NOT NULL DEFAULT 0'), ('auto_approve_condition', 'NVARCHAR(MAX) NULL'), ('auto_cancel_enabled', 'BIT NOT NULL DEFAULT 0'), ('auto_cancel_condition', 'NVARCHAR(MAX) NULL')]
        ...
        line_cols = [('item_code', 'VARCHAR(100) NULL'), ('warranty_text', 'NVARCHAR(500) NULL'), ('serial_numbers', 'NVARCHAR(1000) NULL'), ('quantity', 'NUMERIC(12, 2) NULL DEFAULT 1.0'), ('unit_price', 'NUMERIC(18, 2) NULL DEFAULT 0.0'), ('amount', 'NUMERIC(18, 2) NULL DEFAULT 0.0')]
        ...
        callback_cols = [('payload_source', "VARCHAR(50) NOT NULL DEFAULT 'MAPPING'"), ('stored_procedure_name', 'VARCHAR(200) NULL')]
...
def startup_event():
    try:
        ...
        for uname, uemail, urole in seed_users:
            ...
            if not u_exists:
                db.add(User(username=uname, email=uemail, hashed_password=get_password_hash('password123'), role=urole, is_active=True))
        db.commit()
        db.query(User).count()
...
def root():
    ...
    return {'status': 'online', 'service': settings.PROJECT_NAME, 'version': settings.VERSION, 'database': f'MS SQL Server ({db_target})', 'database_target': db_target, 'docs': '/docs'}

@app.get('/api/health')
def health_check():
    db_raw = settings.DATABASE_URL
    db_target = db_raw.split('@')[-1] if '@' in db_raw else db_raw
    return {'status': 'healthy', 'database_connected': db_target}