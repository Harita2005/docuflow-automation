...
def get_all_workflow_profiles(db: Session=Depends(get_db)):
    ...
    for p in profiles:
        ...
        p_dict = {'profile_name': p.profile_name, 'workflow_code': wf_code, 'workflow_category': p.workflow_category, 'workflow_type': p.workflow_type, 'description': p.description, 'status': p.status, 'approval_threshold': p.approval_threshold, 'rejection_handling': p.rejection_handling, 'reminder_interval_hours': p.reminder_interval_hours, 'escalation_after_hours': p.escalation_after_hours, 'auto_escalation': p.auto_escalation, 'rule_action': p.rule_action or 'WORKFLOW_ROUTE', 'cancel_reason': p.cancel_reason, 'auto_approve_enabled': bool(p.auto_approve_enabled), 'auto_approve_condition': p.auto_approve_condition, 'auto_cancel_enabled': bool(p.auto_cancel_enabled), 'auto_cancel_condition': p.auto_cancel_condition, 'steps': steps}
...
def get_single_workflow_profile(profile_name: str, db: Session=Depends(get_db)):
    ...
    raw_steps = db.query(WorkflowStepDefinition).filter(WorkflowStepDefinition.profile_name == p.profile_name).order_by(WorkflowStepDefinition.stage_number.asc()).all()
    steps = [format_step_with_checklist(db, st, p.profile_name) for st in raw_steps]
    return {'profile_name': p.profile_name, 'workflow_code': p.workflow_code, 'workflow_category': p.workflow_category, 'workflow_type': p.workflow_type, 'description': p.description, 'status': p.status, 'approval_threshold': p.approval_threshold, 'rejection_handling': p.rejection_handling, 'reminder_interval_hours': p.reminder_interval_hours, 'escalation_after_hours': p.escalation_after_hours, 'auto_escalation': p.auto_escalation, 'rule_action': p.rule_action or 'WORKFLOW_ROUTE', 'cancel_reason': p.cancel_reason, 'auto_approve_enabled': bool(p.auto_approve_enabled), 'auto_approve_condition': p.auto_approve_condition, 'auto_cancel_enabled': bool(p.auto_cancel_enabled), 'auto_cancel_condition': p.auto_cancel_condition, 'steps': steps}
...
def delete_workflow_profile(profile_name: str, db: Session=Depends(get_db)):
    ...
    decoded = urllib.parse.unquote(raw_name)
    p = db.query(WorkflowProfile).filter((WorkflowProfile.profile_name == raw_name) | (WorkflowProfile.profile_name == decoded) | WorkflowProfile.profile_name.ilike(raw_name) | WorkflowProfile.profile_name.ilike(decoded)).filter(WorkflowProfile.is_deleted == False).first()
    if not p:
...
def save_workflow_step(payload: dict, db: Session=Depends(get_db)):
    ...
    if not profile_name:
        ...
        if wf_code:
            prof = db.query(WorkflowProfile).filter((WorkflowProfile.workflow_code == str(wf_code)) | (WorkflowProfile.profile_name == str(wf_code))).first()
            if prof:
