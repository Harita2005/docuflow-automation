...
def get_users(include_inactive: bool=Query(True, description='Include deactivated employees'), role: Optional[str]=Query(None, description='Filter by role (admin, manager, finance_auditor, employee)'), search: Optional[str]=Query(None, description='Search by name, employee_id, or email'), db: Session=Depends(get_db)):
    ...
    if search:
        s = f'%{search}%'
        query = query.filter(User.employee_name.ilike(s) | User.employee_id.ilike(s) | User.email.ilike(s) | User.username.ilike(s))
...
def get_user_by_id(user_identifier: str, db: Session=Depends(get_db)):
    ...
    if not user:
        user = db.query(User).filter((User.employee_id == user_identifier) | (User.user_uid == user_identifier) | (User.username == user_identifier)).filter(User.is_deleted == False).first()
    if not user:
...
def create_user_master(payload: UserMasterCreate, db: Session=Depends(get_db), admin: Optional[User]=Depends(get_current_user)):
    ...
    new_user = User(user_uid=user_uid, employee_id=payload.employee_id, employee_name=payload.employee_name, name=payload.employee_name, username=effective_username, email=payload.email, phone_number=payload.phone_number, division=payload.division or 'VCC', department=payload.department, plant=payload.plant, role=payload.role, password_hash=hashed_pwd, is_active=payload.is_active, mfa_enabled=payload.mfa_enabled, mfa_type=payload.mfa_type, created_by=admin.name if admin else payload.created_by or 'System Admin', created_at=datetime.datetime.utcnow())
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user
...
def update_user_master(user_id: int, payload: UserMasterUpdate, db: Session=Depends(get_db)):
...
def toggle_user_status(user_id: int, payload: UserStatusToggleRequest, db: Session=Depends(get_db)):
    user = db.query(User).filter(User.id == user_id).filter(User.is_deleted == False).first()
    if not user:
        raise HTTPException(status_code=404, detail='Employee not found')
