...
def get_audit_logs(invoice_id: Optional[str]=Query(None), db: Session=Depends(get_db)):
...
def get_system_logs(invoice_id: Optional[str]=Query(None), db: Session=Depends(get_db)):
