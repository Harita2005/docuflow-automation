...
from app.models import Invoice, WorkflowStepDefinition, AuditLog, User, ChecklistTemplate, InvoiceChecklistState, NotificationRaciMatrix, NotificationProviderConfig, ChecklistRule, InAppNotification
...
async def upload_document(file: UploadFile=File(...), division: Optional[str]=Form('VCC'), plant: Optional[str]=Form('TN-SIVAKASI'), document_type: Optional[str]=Form('AP INVOICE'), db: Session=Depends(get_db)):
    import uuid
...
def resolve_checklist_items(db: Session, inv: Invoice, stage_name: str) -> List[str]:
    ...
    if inv.workflow_profile_id:
        step_def = db.query(WorkflowStepDefinition).filter(WorkflowStepDefinition.profile_name == inv.workflow_profile_id, WorkflowStepDefinition.step_name.ilike(stage_name.strip())).first()
    ...
    if not combined_items and inv.workflow_profile_id:
        tpl_items = db.query(ChecklistTemplate).filter(ChecklistTemplate.workflow_profile == inv.workflow_profile_id, ChecklistTemplate.stage_name.ilike(stage_name.strip()), ChecklistTemplate.is_active == True).order_by(ChecklistTemplate.sequence_order.asc()).all()
        if tpl_items:
            for it in tpl_items:
