...
def record_audit(db: Session, entity_type: str, entity_id: int, action: str, prev: Any=None, new_val: Any=None, user: str='System Admin'):
    try:
        audit = IntegrationAuditHistory(entity_type=entity_type, entity_id=entity_id, action=action, previous_value_json=json.dumps(prev, default=str) if prev else None, new_value_json=json.dumps(new_val, default=str) if new_val else None, changed_by=user, timestamp=datetime.datetime.utcnow())
        db.add(audit)
        db.commit()
...
def list_applications(db: Session=Depends(get_db)):
    ...
    for a in apps:
        ...
        results.append({'id': a.id, 'name': a.name, 'code': a.code, 'description': a.description, 'base_url': a.base_url, 'environment': a.environment or 'Production', 'status': a.status or 'Active', 'auth_type': a.auth_type or 'None', 'auth_config_json': a.auth_config_json, 'rules_count': rules_count, 'last_callback': last_callback_str, 'created_at': a.created_at.isoformat() if a.created_at else None, 'updated_at': a.updated_at.isoformat() if a.updated_at else None})
...
def create_application(body: ThirdPartyApplicationCreate, db: Session=Depends(get_db)):
    ...
    app_obj = ThirdPartyApplication(name=body.name.strip(), code=body.code.strip().upper(), description=body.description, base_url=body.base_url.strip(), environment=body.environment or 'Production', status=body.status or 'Active', auth_type=body.auth_type or 'None', auth_config_json=helper_serialize_json_field(body.auth_config_json), created_at=datetime.datetime.utcnow(), updated_at=datetime.datetime.utcnow())
    db.add(app_obj)
    db.commit()
    db.refresh(app_obj)
...
def list_callback_rules(application_id: Optional[int]=None, db: Session=Depends(get_db)):
    ...
    for r in rules:
        ...
        results.append({'id': r.id, 'rule_name': r.rule_name, 'description': r.description, 'application_id': r.application_id, 'application_name': r.application.name if r.application else 'Unknown', 'application_code': r.application.code if r.application else '', 'status': r.status or 'ACTIVE', 'priority': r.priority or 100, 'trigger_event': r.trigger_event or 'FDO_FINAL_DECISION', 'run_when': r.run_when or 'BOTH', 'conditions_json': r.conditions_json, 'http_method': r.http_method or 'POST', 'url_mode': r.url_mode or 'INHERIT_BASE', 'endpoint_path': r.endpoint_path, 'custom_url': r.custom_url, 'body_type': r.body_type or 'JSON', 'content_type': r.content_type or 'application/json', 'payload_mapping_json': r.payload_mapping_json, 'raw_payload_template': r.raw_payload_template, 'query_params_json': r.query_params_json, 'headers_json': r.headers_json, 'auth_override_type': r.auth_override_type or 'INHERIT', 'auth_override_config_json': r.auth_override_config_json, 'timeout_seconds': r.timeout_seconds or 30, 'success_criteria_json': r.success_criteria_json, 'follow_redirects': bool(r.follow_redirects), 'retry_config_json': r.retry_config_json, 'last_execution': last_exec_str, 'created_at': r.created_at.isoformat() if r.created_at else None, 'updated_at': r.updated_at.isoformat() if r.updated_at else None})
    return {'success': True, 'count': len(results), 'data': results}
...
def create_callback_rule(body: CallbackRuleCreate, db: Session=Depends(get_db)):
    ...
    rule = CallbackRule(rule_name=body.rule_name.strip(), description=body.description, application_id=body.application_id, status=body.status or 'ACTIVE', priority=body.priority if body.priority is not None else 100, trigger_event=body.trigger_event or 'FDO_FINAL_DECISION', run_when=body.run_when or 'BOTH', conditions_json=helper_serialize_json_field(body.conditions_json), http_method=(body.http_method or 'POST').upper(), url_mode=body.url_mode or 'INHERIT_BASE', endpoint_path=body.endpoint_path, custom_url=body.custom_url, body_type=body.body_type or 'JSON', payload_source=body.payload_source or 'MAPPING', stored_procedure_name=body.stored_procedure_name, content_type=body.content_type or 'application/json', payload_mapping_json=helper_serialize_json_field(body.payload_mapping_json), raw_payload_template=body.raw_payload_template, query_params_json=helper_serialize_json_field(body.query_params_json), headers_json=helper_serialize_json_field(body.headers_json), auth_override_type=body.auth_override_type or 'INHERIT', auth_override_config_json=helper_serialize_json_field(body.auth_override_config_json), timeout_seconds=body.timeout_seconds or 30, success_criteria_json=helper_serialize_json_field(body.success_criteria_json), follow_redirects=bool(body.follow_redirects), retry_config_json=helper_serialize_json_field(body.retry_config_json), created_at=datetime.datetime.utcnow(), updated_at=datetime.datetime.utcnow())
    db.add(rule)
    db.commit()
    db.refresh(rule)
...
def duplicate_callback_rule(rule_id: int, db: Session=Depends(get_db)):
    ...
    if not rule:
        raise HTTPException(status_code=404, detail=f'Callback rule #{rule_id} not found')
    new_rule = CallbackRule(rule_name=f'{rule.rule_name} (Copy)', description=rule.description, application_id=rule.application_id, status='DRAFT', priority=rule.priority, trigger_event=rule.trigger_event, run_when=rule.run_when, conditions_json=rule.conditions_json, http_method=rule.http_method, url_mode=rule.url_mode, endpoint_path=rule.endpoint_path, custom_url=rule.custom_url, body_type=rule.body_type, content_type=rule.content_type, payload_mapping_json=rule.payload_mapping_json, raw_payload_template=rule.raw_payload_template, query_params_json=rule.query_params_json, headers_json=rule.headers_json, auth_override_type=rule.auth_override_type, auth_override_config_json=rule.auth_override_config_json, timeout_seconds=rule.timeout_seconds, success_criteria_json=rule.success_criteria_json, follow_redirects=rule.follow_redirects, retry_config_json=rule.retry_config_json, created_at=datetime.datetime.utcnow(), updated_at=datetime.datetime.utcnow())
    db.add(new_rule)
    db.commit()
    db.refresh(new_rule)
...
def execute_test_callback(body: TestCallbackRequest, db: Session=Depends(get_db)):
    ...
    if not app_obj:
        app_obj = ThirdPartyApplication(name='Sample Target Application', code='SAMPLE_APP', base_url='https://httpbin.org', auth_type='None')
    if not rule_obj and body.rule_config:
        cfg = body.rule_config
        rule_obj = CallbackRule(rule_name=cfg.rule_name, application_id=app_obj.id or 1, http_method=cfg.http_method or 'POST', url_mode=cfg.url_mode or 'INHERIT_BASE', endpoint_path=cfg.endpoint_path or '/post', custom_url=cfg.custom_url, body_type=cfg.body_type or 'JSON', content_type=cfg.content_type or 'application/json', payload_mapping_json=helper_serialize_json_field(cfg.payload_mapping_json), raw_payload_template=cfg.raw_payload_template, query_params_json=helper_serialize_json_field(cfg.query_params_json), headers_json=helper_serialize_json_field(cfg.headers_json), auth_override_type=cfg.auth_override_type or 'INHERIT', auth_override_config_json=helper_serialize_json_field(cfg.auth_override_config_json), timeout_seconds=cfg.timeout_seconds or 10)
    if not rule_obj:
        rule_obj = CallbackRule(rule_name='Default Test Callback Rule', application_id=app_obj.id or 1, http_method='POST', url_mode='INHERIT_BASE', endpoint_path='/post', body_type='JSON')
    sample_ctx = {'primaryKey': body.sample_primary_key or 'TEST-84932', 'documentNumber': body.sample_document_number or 'TEST-INV-1024', 'approvalStatus': (body.sample_approval_status or 'APPROVED').upper(), 'company': body.sample_company or 'VCC', 'documentType': body.sample_document_type or 'AP INVOICE', 'category': 'Raw Materials', 'branch': 'TN-PLANT-01', 'costCenter': 'FIN-CC-102', 'approvedBy': 'QA Test Admin', 'approvalDate': datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'), 'rejectionReason': 'Test rejection notice' if (body.sample_approval_status or '').upper() == 'REJECTED' else '', 'eventId': f'TEST-EVT-{int(time.time())}'}
    try:
        method, final_url, final_headers, body_bytes = build_callback_request(rule_obj, app_obj, sample_ctx, db=db)
        request_preview = {'method': method, 'url': final_url, 'headers': mask_sensitive_headers(final_headers), 'body': body_bytes.decode('utf-8', errors='ignore') if body_bytes else None}
...
@router.get('/logs')
def list_integration_logs(application_id: Optional[int]=None, rule_id: Optional[int]=None, status_filter: Optional[str]=None, decision: Optional[str]=None, search: Optional[str]=None, limit: int=Query(50, ge=1, le=200), db: Session=Depends(get_db)):
    ...
    if search:
        s = f'%{search.strip()}%'
        query = query.filter(CallbackEvent.event_id.ilike(s) | CallbackEvent.document_number.ilike(s) | CallbackEvent.source_primary_key.ilike(s))
    events = query.order_by(CallbackEvent.created_at.desc()).limit(limit).all()
    data = []
    for e in events:
        data.append({'id': e.id, 'event_id': e.event_id, 'document_id': e.document_id, 'document_number': e.document_number or e.document_id, 'primary_key': e.source_primary_key or e.document_id, 'application_id': e.application_id, 'application_name': e.application.name if e.application else 'Unknown', 'rule_id': e.rule_id, 'rule_name': e.rule.rule_name if e.rule else 'Unknown', 'decision': e.decision, 'method': e.rule.http_method if e.rule else 'POST', 'status': e.status, 'attempt_count': e.attempt_count, 'max_attempts': e.max_attempts, 'next_retry_at': e.next_retry_at.isoformat() if e.next_retry_at else None, 'created_at': e.created_at.isoformat() if e.created_at else None, 'updated_at': e.updated_at.isoformat() if e.updated_at else None})
...
def get_integration_log_detail(event_id: str, db: Session=Depends(get_db)):
    ...
    attempts_data = []
    for a in attempts:
        attempts_data.append({'id': a.id, 'attempt_number': a.attempt_number, 'http_method': a.http_method, 'request_url': a.request_url, 'request_headers': a.request_headers_json, 'request_body': a.request_body, 'response_status_code': a.response_status_code, 'response_headers': a.response_headers_json, 'response_body': a.response_body, 'response_time_ms': a.response_time_ms, 'status': a.status, 'error_message': a.error_message, 'timestamp': a.timestamp.isoformat() if a.timestamp else None})
    return {'success': True, 'event': {'id': evt.id, 'event_id': evt.event_id, 'document_id': evt.document_id, 'document_number': evt.document_number, 'primary_key': evt.source_primary_key, 'decision': evt.decision, 'status': evt.status, 'attempt_count': evt.attempt_count, 'max_attempts': evt.max_attempts, 'created_at': evt.created_at.isoformat() if evt.created_at else None, 'application_name': evt.application.name if evt.application else '', 'rule_name': evt.rule.rule_name if evt.rule else ''}, 'attempts': attempts_data}
...
def manual_retry_callback(event_id: str, db: Session=Depends(get_db)):
    ...
    return {'success': is_ok, 'event_id': str(evt.event_id), 'message': 'Manual retry executed successfully' if is_ok else 'Manual retry attempt failed', 'result': {'status': 'success' if is_ok else 'failed'}}
...
def bulk_retry_failed_callbacks(db: Session=Depends(get_db)):
    ...
    for evt in failed_events:
        res = execute_callback_event(db, evt.id)
        is_ok = bool(res.get('success', False)) if isinstance(res, dict) else False
        results.append({'event_id': str(evt.event_id), 'success': is_ok, 'result': {'status': 'success' if is_ok else 'failed'}})
    return {'success': True, 'total_retried': len(failed_events), 'successful_count': sum((1 for r in results if r['success'])), 'failed_count': sum((1 for r in results if not r['success'])), 'results': results}
...
def get_audit_history(limit: int=50, db: Session=Depends(get_db)):
    ...
    return {'success': True, 'data': [{'id': a.id, 'entity_type': a.entity_type, 'entity_id': a.entity_id, 'action': a.action, 'previous_value': a.previous_value_json, 'new_value': a.new_value_json, 'changed_by': a.changed_by, 'timestamp': a.timestamp.isoformat() if a.timestamp else None} for a in audits]}