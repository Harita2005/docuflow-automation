...
five_items = ['Documents Attached', 'Bill Name Verified', 'Bill Date Verified', 'Party Name & Total Amount Verified', 'Vendor GST no, Signaure Verified']
print('==========================================================')
print('  UPDATING CHECKLIST ITEMS TO MATCH EXACT 5 USER ITEMS')
print('==========================================================')
step = db.query(WorkflowStepDefinition).filter(WorkflowStepDefinition.profile_name == wf_name, WorkflowStepDefinition.stage_number == 1).first()
if step:
...
for idx, item in enumerate(five_items, 1):
    rule = ChecklistRule(rule_name=wf_name, division='VCC', category='Vendor Payment Workflows', workflow_profile=wf_name, stage_name=stage, item_text=item, is_mandatory=True, is_active=True, sequence_order=idx)
    db.add(rule)
...
for idx, item in enumerate(five_items, 1):
    tmpl = ChecklistTemplate(workflow_profile=wf_name, stage_name=stage, item_text=item, is_mandatory=True, is_active=True, sequence_order=idx)
    db.add(tmpl)
...
for item in five_items:
    st = DocumentChecklistState(invoice_id='DOC-VCC-001', stage_name=stage, item_text=item, is_checked=False)
    db.add(st)
