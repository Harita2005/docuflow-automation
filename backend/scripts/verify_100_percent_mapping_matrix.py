...
from app.models import User, WorkflowProfile, BusinessRule, Invoice, DocumentChecklistState
...
try:
    ...
    for idx, rule in enumerate(active_rules, 1):
        ...
        doc_dict = {'division': 'VCC', 'plant': 'PERUNDURAI', 'category': 'Freight Charges', 'cost_center': 'DIGITAL MARKETING', 'vendor_name': 'Standard Vendor', 'amount': 50000.0, 'tax_amount': 9000.0, 'pay_mode': 'BANK', 'document_type': rule.document_type or 'AP INVOICE'}
        for c in conditions:
            f = c.get('field', '')
            v = c.get('value', '')
        ...
        mock_inv = Invoice(division=doc_dict.get('division') or 'VCC', plant=doc_dict.get('plant') or 'TN-ERODE-PERUNDURAI ROAD', category=doc_dict.get('category') or 'Freight Charges', cost_center=doc_dict.get('cost_center') or 'DIGITAL MARKETING', vendor_name=doc_dict.get('vendor_name') or 'Standard Vendor', amount=float(doc_dict.get('amount') or 50000.0), tax_amount=float(doc_dict.get('tax_amount') or 9000.0), pay_mode=doc_dict.get('pay_mode') or 'BANK', document_type=doc_dict.get('document_type') or 'AP INVOICE')
    ...
    sample_requests = [DocumentSyncRequest(company_code='VCC', invoice_number=f'INV-MATRIX-TEST-1001', doc_key='DOC-MATRIX-KEY-1001', document_type='AP INVOICE', category='Freight Charges', cost_center='DIGITAL MARKETING', plant='TN-ERODE-PERUNDURAI ROAD', amount=185000.0, vendor_code='VEND-1001', vendor_name='VCC Express Logistics', pay_mode='BANK'), DocumentSyncRequest(company_code='ACC', invoice_number=f'INV-MATRIX-TEST-1002', doc_key='DOC-MATRIX-KEY-1002', document_type='AP INVOICE', category='ACCESSORIES - COMPUTER', cost_center='Office Maintenance', plant='ATITHYA-EXCISE', amount=250000.0, vendor_code='VEND-1002', vendor_name='Dell Enterprise Hardware', pay_mode='BANK'), DocumentSyncRequest(company_code='ATC', invoice_number=f'INV-MATRIX-TEST-1003', doc_key='DOC-MATRIX-KEY-1003', document_type='AP INVOICE', category='MACHINERY', cost_center='PRODUCTION TABLE', plant='BHAVANI', amount=950000.0, vendor_code='VEND-1003', vendor_name='Global Heavy Equipment Ltd', pay_mode='BANK')]
