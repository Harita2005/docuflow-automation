...
db.commit()
doc = Document(id=doc_id, doc_key='ERP-VCC-ITHW-1002', doc_num='VCC-ITHW-002', doc_date='2026-09-03', party_name='DELL INDIA PRIVATE LIMITED', party_code='V-DELL-002', party_tax_id='29AAACD2026E1Z5', vendor_name='DELL INDIA PRIVATE LIMITED', vendor_code='V-DELL-002', vendor_gstin='29AAACD2026E1Z5', invoice_number='INV-VCC-ITHW-002', invoice_date='2026-09-03', po_number='PO-VCC-ITHW-2026', amount=85000.0, base_amount=72033.9, tax_amount=12966.1, cgst=6483.05, sgst=6483.05, currency='INR', document_type='AP INVOICE', division='vcc', category='IT Hardware Purchase', cost_center='it-hardware', plant='HO-CHENNAI', payment_terms='Net 30', pay_mode='BANK', status='Pending Approval', current_stage=1, is_deleted=False, created_at=datetime.datetime.utcnow())
...
db.add(doc)
line1 = DocumentLineItem(invoice_id=doc_id, description='Dell OptiPlex 7090 Desktop & Peripherals', quantity=1.0, unit_price=72033.9, amount=72033.9, item_code='HW-DELL-01')
db.add(line1)
audit = DocumentApprovalLog(invoice_id=doc_id, user='System Ingestion', action='DOCUMENT_CREATED', stage='Stage 1 - Attachment Status', notes=f"Ingested document Division: vcc, Cost Center: it-hardware. Matched Rule: {(rule_eval.get('rule_name') if rule_eval else 'None')}")
db.add(audit)
db.commit()
print('==========================================================')
