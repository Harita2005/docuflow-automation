...
if not rule:
    conditions = [{'field': 'Division', 'operator': 'EQUALS', 'value': 'VCC'}, {'field': 'Cost Center', 'operator': 'EQUALS', 'value': new_cc}, {'field': 'Total Amount', 'operator': 'GREATER_THAN', 'value': '0'}]
    rule = BusinessRule(rule_name=rule_name, rule_category='Vendor Payment Workflows', document_type='AP INVOICE', priority=10, target_workflow_id=wf_name, conditions_json=json.dumps(conditions), rule_action='WORKFLOW_ROUTE', is_active=True)
    db.add(rule)
    db.commit()
...
if existing_doc:
    ...
    db.commit()
doc2 = Document(id=doc_id, doc_key=doc_id, doc_num='INV-VCC-2026-002', doc_date=datetime.date.today().strftime('%Y-%m-%d'), party_name='SIEMENS AUTOMATION INDIA LTD', vendor_name='SIEMENS AUTOMATION INDIA LTD', invoice_number='INV-VCC-2026-002', amount=85000.0, base_amount=75000.0, tax_amount=10000.0, currency='INR', document_type='AP INVOICE', division='VCC', cost_center=new_cc, category='Vendor Payment Workflows', status='Pending Approval', current_stage=1, created_at=datetime.datetime.utcnow())
db.add(doc2)
db.commit()
