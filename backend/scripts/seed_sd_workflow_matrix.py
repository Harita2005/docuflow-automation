...
candidate_paths = [BACKEND_DIR / 'data' / 'SD Checklists.xlsx', BACKEND_DIR / 'data' / 'SD SCHEMA AND WORKFLOW DETAILS.xlsx', BASE_DIR / 'SD Checklists.xlsx', Path('/app/data/SD Checklists.xlsx'), Path('/app/SD Checklists.xlsx'), Path.cwd() / 'SD Checklists.xlsx', BACKEND_DIR / 'SD Checklists.xlsx']
...
def seed_sd_workflow_matrix():
    ...
    try:
        with zipfile.ZipFile(str(EXCEL_PATH), 'r') as z:
            ...
            all_profiles = {}
            rule_groups = defaultdict(lambda: {'branches': set(), 'costcenters': set(), 'paymodes': set()})
            for sname in ['SD WORKFLOW', 'VCC WORKFLOW']:
                ...
                for row in rows_elem[1:]:
                    ...
                    if sname == 'SD WORKFLOW':
                        stage_cols = [('Attachment Status', cells.get('G')), ('First Approval', cells.get('H')), ('Second Approval', cells.get('I')), ('3rd APPROVAL', cells.get('J')), ('IA Approval', cells.get('K')), ('Final Approval', cells.get('L'))]
                    else:
                        stage_cols = [('Attachment Status', cells.get('G')), ('First Approval', cells.get('H')), ('Second Approval', cells.get('I')), ('IA Approval', cells.get('J')), ('Final Approval', cells.get('K')), ('3rd APPROVAL', cells.get('L'))]
                    active_steps = []
                    ...
                    if cat_name not in all_profiles and active_steps:
                        ...
                        all_profiles[cat_name] = {'profile_name': cat_name, 'workflow_code': cat_name[:50], 'workflow_category': cat_label, 'workflow_type': doc_type_inferred, 'description': f'Automated matrix workflow for {cat_name} ({company})', 'steps': [(idx + 1, st_name, pool) for idx, (st_name, pool) in enumerate(active_steps)]}
        ...
        for p_name, p_data in all_profiles.items():
            ...
            if not existing_p:
                new_p = WorkflowProfile(profile_name=p_name, workflow_code=p_data['workflow_code'], workflow_category=p_data['workflow_category'], workflow_type=p_data['workflow_type'], description=p_data['description'], status='Active', approval_threshold=100, rejection_handling='Return to Previous Step', reminder_interval_hours=24, escalation_after_hours=48, auto_escalation=False)
                db.add(new_p)
                db.flush()
                synced_p += 1
            ...
            for stage_num, st_name, pool in p_data['steps']:
                ...
                db.add(WorkflowStepDefinition(profile_name=p_name, stage_number=stage_num, step_name=st_name, approver_type='Specific Employee', approver_target=target, document_type=p_data['workflow_type'], action_required='Approve', permissions='Approve / Reject', sla_hours=48))
                synced_s += 1
        db.commit()
        ...
        consolidated_groups = defaultdict(lambda: {'companies': set(), 'categories': set(), 'branches': set(), 'costcenters': set(), 'doc_type': ''})
        for (cat_name, company, category), data in rule_groups.items():
            ...
            for b in data['branches']:
                if b and b != 'ALL':
                    consolidated_groups[grp_key]['branches'].add(b)
            for cc in data['costcenters']:
                if cc and cc != 'ALL':
                    consolidated_groups[grp_key]['costcenters'].add(cc)
        ...
        for (cat_name, parent_comp), data in consolidated_groups.items():
            ...
            conditions = [{'field': 'Division', 'operator': 'contains any of' if ',' in comp_val else 'equals', 'value': comp_val, 'logicalOperator': 'AND'}]
            ...
            generated_rules.append({'rule_name': rule_name, 'rule_category': rule_cat, 'document_type': doc_type_for_rule, 'priority': priority, 'target_workflow_id': cat_name, 'conditions_json': json.dumps({'conditions': conditions}), 'description': f'Consolidated rule for {parent_comp} -> {cat_name} ({len(categories)} categories, {len(branches)} branches, {len(costcenters)} cost centers)', 'is_active': True})
        ...
        for r_dict in generated_rules:
            new_r = BusinessRule(rule_name=r_dict['rule_name'], rule_category=r_dict['rule_category'], document_type=r_dict['document_type'], priority=r_dict['priority'], target_workflow_id=r_dict['target_workflow_id'], conditions_json=r_dict['conditions_json'], description=r_dict['description'], is_active=True)
            db.add(new_r)
        db.commit()
