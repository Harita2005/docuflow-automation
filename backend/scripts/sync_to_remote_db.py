...
TABLES_ORDERED = ['users', 'workflow_profiles', 'workflow_step_definitions', 'business_rules', 'checklist_templates', 'checklist_rules', 'documents', 'document_line_items', 'document_checklist_states', 'document_approval_logs', 'system_engine_logs', 'in_app_notifications', 'notification_provider_configs', 'notification_raci_matrices']
        ...
        if 'mssql' in remote_db_url or 'sqlserver' in remote_db_url:
            ...
            for tbl in reversed(TABLES_ORDERED):
                try:
                    ...
                    import logging
                    logging.getLogger(__name__).debug('Handled exception: %s', wipe_err)
