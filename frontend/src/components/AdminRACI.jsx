...
const LEGACY_EVENTS = ["Approve", "Reject", "Request Clarification", "Send Back", "Escalate"];
...
export default function AdminRACI() {
  ...
  useEffect(() => {
    ...
    EVENTS.forEach(event => {
      let match = matrices.find(m => m.workflow_profile === activeWorkflow && m.event_name === event);
      if (!match) {
        match = matrices.find(m => m.workflow_profile === activeWorkflow);
      }
  ...
  const saveRACI = async () => {
    ...
    try {
      const eventsToSave = [UNIFIED_EVENT, ...LEGACY_EVENTS];
      for (const event of eventsToSave) {
        const sourceData = currentGrid[UNIFIED_EVENT] || { R: [], A: [], C: [], I: [], title_template: '', message_template: '' };
        const payload = {
          workflow_profile: activeWorkflow,
          event_name: event,
          responsible_emails: JSON.stringify(sourceData.R),
          accountable_emails: JSON.stringify(sourceData.A),
          consulted_emails: JSON.stringify(sourceData.C),
          informed_emails: JSON.stringify(sourceData.I),
          title_template: sourceData.title_template,
          message_template: sourceData.message_template
        };
  ...
  const testSMTPConnection = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Error sending test email: ' + (err.detail || err.error || 'Unknown error'));
      }
    } catch(e) {
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/80 border-b border-slate-200 text-[8.5px] font-black text-slate-400 uppercase tracking-wider">
              <th className="py-2.5 px-3.5 w-48">Workflow RACI Matrix</th>
              <th className="py-2.5 px-3.5 w-1/4">
                <span className="text-blue-600 font-black">R</span>ESPONSIBLE
                <span className="text-[7.5px] text-slate-500 block font-semibold">Action Owner • Initial CC</span>
              </th>
              <th className="py-2.5 px-3.5 w-1/4">
                <span className="text-purple-600 font-black">A</span>CCOUNTABLE
                <span className="text-[7.5px] text-slate-500 block font-semibold">Outcome Owner • Initial CC</span>
              </th>
              <th className="py-2.5 px-3.5 w-1/4">
                <span className="text-amber-600 font-black">C</span>ONSULTED
                <span className="text-[7.5px] text-slate-500 block font-semibold">SME Provider • Initial CC</span>
              </th>
              <th className="py-2.5 px-3.5 w-1/4">
                <span className="text-emerald-600 font-black">I</span>NFORMED
                <span className="text-[7.5px] text-slate-500 block font-semibold">In Loop • Initial CC</span>
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 text-[10px]">
            {EVENTS.map(event => (
              <tr key={event} className="hover:bg-slate-50/60 transition">
                <td className="py-3 px-3.5 align-top font-bold text-slate-900 bg-slate-50/40 border-r border-slate-100">
                  <div className="text-xs font-black text-slate-800">Workflow Initiation</div>
                  <div className="text-[9px] font-medium text-slate-500 mt-0.5">Initial Flow Start Stakeholders</div>
                  <button
                    type="button"
                    onClick={() => setActiveModalEvent(event)}
                    className="mt-2.5 px-2.5 py-1 bg-white border border-slate-200 hover:border-blue-400 hover:bg-blue-50/50 text-blue-700 rounded-md text-[9px] font-bold transition inline-flex items-center gap-1.5 cursor-pointer shadow-2xs"
                  >
                ...
                {['R', 'A', 'C', 'I'].map(role => (
                  <td key={role} className="py-2.5 px-3 align-top border-l border-slate-100">
                    <div className="flex flex-col gap-2">
                      {/* Active User/Role Tags */}
                      <div className="flex flex-wrap gap-1">
                        {grid[event] && grid[event][role] && grid[event][role].map(item => (
                          <span key={item} className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-blue-50/80 border border-blue-200/90 rounded-md text-[9px] font-semibold text-blue-950 shadow-2xs">
                            <span className="text-[10px]">👤</span>
                            <span>{item}</span>
                            <button onClick={() => handleRemoveEmail(event, role, item)} className="text-blue-400 hover:text-red-600 transition cursor-pointer ml-0.5">
                              <X className="h-3 w-3" />
                            </button>
                          </span>
                        ))}
                      ...
                      {/* Dropdown Selector for DB Users & Roles */}
                      <div className="space-y-1">
                        <select
                          className="w-full text-[9px] font-semibold px-2 py-0.5 bg-slate-50 border border-slate-200 rounded focus:border-blue-500 outline-hidden h-6 cursor-pointer text-slate-800"
                          value=""
                          onChange={(e) => {
                            if (e.target.value) {
                              handleAddEmail(event, role, e.target.value);
                              e.target.value = '';
                            }
                          }}
                        >
                          <option value="">+ Add User or Role from DB...</option>
                          <optgroup label="System Users (DB)">
                            {dbUsers.map(u => (
                              <option key={u.id} value={`${u.employee_name || u.username} (${u.email})`}>
                                {u.employee_name || u.name || u.username} ({u.email})
                              </option>
                            ))}
                          </optgroup>
                          <optgroup label="System Roles">
                            <option value="Finance Department Lead">Finance Department Lead</option>
                            <option value="Internal Auditor">Internal Auditor</option>
                            <option value="Division GM / Executive">Division GM / Executive</option>
                            <option value="Accounts Payable Executive">Accounts Payable Executive</option>
                          </optgroup>
                        </select>

                        {/* Custom Name / Email Text Input */}
                        <div className="flex items-center gap-1">
                          <input 
                            id={`input-${event}-${role}`}
                            type="text" 
                            placeholder="Or type custom name/email..." 
                            className="w-full text-[8.5px] font-mono px-2 py-0.5 bg-slate-50 border border-slate-200 rounded focus:border-blue-500 outline-hidden h-5.5"
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                e.preventDefault();
                          ...
                          />
                          <button 
                            className="p-0.5 px-1.5 bg-slate-100 hover:bg-blue-100 text-slate-600 hover:text-blue-700 border border-slate-200 rounded shrink-0 transition cursor-pointer text-[9px] font-bold"
